...
本章では、5章で説明した背景をもとに開発したプログラミング言語mimium(**mi**nimal-**m**usical-med**ium**)[^mimiumrepo]を設計した。の具体的な実装について記す。

[^mimiumrepo]: 開発時点でのバージョンは0.4.0をもとに解説している。ソースコードは<https://github.com/mimium-org/mimium>で公開されている。


\begin{table*}[htb]
  \caption{Comparison of specifications between computer music languages.}
...
以下のセクションでは、mimiumの言語仕様と実行環境の詳細を記述する。
