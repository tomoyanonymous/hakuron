...
このPDP-224ではすでに、パンチカードを用いず、CRTディスプレイとタイプライターを用いた簡易的なテキスト表示による対話的プログラミングが実現できた（また、波形をリアルタイムで描画もできるオシロスコープ機能も使われている）。波形生成ステップでは、まず、3種類の基本的な波形生成の宣言の文法を用いて、基本波形を生成する。波形生成はステップ関数、ランプ（折れ線）関数、すでに作られた波形からの切り取り
