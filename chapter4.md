...
## MUSIC 11、Csound、SAOL

サムソンが去った後のMITでは、1973年にExperimental Music Studio（EMS）という電子音楽の研究所がバリー・ヴァーコーらによって設立される[^mitems]。ヴァーコーはここでPDP-11向けのMUSICシリーズの移植、MUSIC 11を開発した。以前にヴァーコーが開発していたIBM 360向けのMUSIC 360と比べ、MUSIC 11では、signal/controlの明示的な区分という、必要な処理の時間解像度に合わせて負荷を軽減できる仕組みが導入された。例えばビブラートの表現に使うオシレーターに使われる周波数は0~20Hz程度の範囲内に収まるが、これを他の音声信号に使う1秒間に48000回程度の解像度で計算するのは効率が悪い。そこで、低い周波数でしか用いないUGenは別の低いサンプリング周波数で扱うことにしたのだ。この区別は今日のSuperColliderなどの構成にも大きく影響を与えた。

ヴァーコーは、2012年のインタビューで、この区別の着想元として、ブックラのモジュラーシンセサイザーの仕組みを挙げている[@Vercoe2012]。モーグをはじめとした多くのモジュラーシンセサイザーは、基本的にすべての信号を1つの種類のコネクタでパッチするに対し、ブックラはコントロールに扱う信号のパッチングにバナナプラグという異なる種類のコネクタを利用する。この音声/制御信号区別は、複数の同じようなオシレーターを用いるとしても、それを音声信号のソースと何かパラメーターの制御に用いるものという2つに分けるため、本質的に可能な表現の範囲が変わらないとしてもシンセサイザーの演奏者やMUSIC11のプログラマのメンタルモデルにも大きく影響を与えるものである。
...
MUSIC 11は初めてsignal/controlの明示的な区分という、必要な処理の時間解像度に合わせて負荷を軽減できる仕組みを導入した。

ヴァーコーはMUSIC 11をベースに、1986年にC言語を用いてCsoundを実装する。
