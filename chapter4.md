...
このような、フォーマットやプロトコル、あるいはメディアやインフラストラクチャとしてのPLfMの形成過程は、この頃のリアルタイムCsoundの発展の中に見出すことができる。
...
その後、1994年にパケットはカリフォルニア大学サンディエゴ校に着任し、新たに無料でオープンソースかつ、特別なDSPボードを必要とせずチルダオブジェクトを用いる信号処理が可能なソフトウェア、PureDataを開発する[@Puckette1996]。

この、PureDataの信号処理機能はジカレリによって商用化Maxに "MSP"という機能として取り込まれ、ソフトウェア名もMax/MSPとして発売されるようになる[^maxname]。

[^maxname]: その後の映像処理環境Jitterの追加に伴いMax/MSP+Jitterという名前で販売されていたが、2009年のバージョン6以降は単にMaxと呼称されるようになる。

パケットが去った後のIRCAM内では、NeXT社がAppleに吸収されたことや、CPUベースでもリアルタイム処理が十分に可能になったことを契機として、新たにJava言語を利用して開発されたオープンソースソフトウェアのjMaxが公開される。jMaxは2004年までで開発が停止され、2008年に一度jMax Phoenixとして開発が再開されるも2011年時点で再び更新が停止されている。
