...
[^linuxscheduling]: Linuxカーネルの場合にはユーザーはプロセスのスケジューリングポリシーをいくつかの種類から選択できる。 https://linuxjm.osdn.jp/html/LDP_man-pages/man7/sched.7.html 2022年1月31日最終閲覧。またそもそもLinuxの場合はOSがオープンソースなため、Bela（[第1章](../chapter1_rendered#sec:intro)参照）で用いられていたXenomaiのように、ユーザーアプリケーションがメインのスケジューラーをバイパスして動作するカーネル拡張を利用する選択肢もある。
...
[^sigplan]: 筆者の投稿したFARMなどの国際会議を主催する団体。[第2章](../chapter2_rendered#sec:researchfield)を参照。
...
![Faustのような、粒度の細かい中間表現を用いる言語の特性を表した図。](../img/tradeoff4.png){#fig:tradeoff4}

![粒度の小さな中間表現の言語で動的変更をしやすくしようとすると開発コストが増大することを示した図。](../img/tradeoff5.png){#fig:tradeoff5}
