...
代わりに必要となるのは、音楽のために、音楽に関係ないと思われてきたテクノロジーを作り直すこと––音楽土木工学である。
...
グリッチに関しても同様である。大抵のモダンなファイルフォーマットにはパリティと呼ばれる、データが破損しているかチェックするための仕組みが折り込まれている。その結果起きることは、次のようになる。バイナリエディタを開き適当にデータを破壊して上書き保存。ファイルを開く。「このファイルは壊れています」と表示されプログラムは終了する。もはや、テクノロジーの中身を知らないままに誤用を行うことはできなくなってしまったのだ。
